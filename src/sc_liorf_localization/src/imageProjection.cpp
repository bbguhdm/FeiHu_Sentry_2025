...
// <!-- sc_liorf_localization_localization_yjz_lucky_boy -->
...
private:
    ...
    rclcpp::Subscription<livox_ros_driver2::msg::CustomMsg>::SharedPtr subCustomCloud;
    ...
    std::deque<livox_ros_driver2::msg::CustomMsg> CustomQueue;
    sensor_msgs::msg::PointCloud2 currentCloudMsg;
    livox_ros_driver2::msg::CustomMsg currentCustomMsg;
    ...
    pcl::PointCloud<LiovxPointCustomMsg>::Ptr customCloudIn;
    ...
    pcl::PointCloud<LiovxPointCustomMsg>::Ptr tmpLivoxCMCloudIn;
...
public:
    ImageProjection(const rclcpp::NodeOptions & options) :
            ParamServer("sc_liorf_localization_imageProjection", options), deskewFlag(0)
    {
        ...
        // subCustomCloud = create_subscription<livox_ros_driver2::msg::CustomMsg>(pointCustomTopic,QosPolicy(history_policy, reliability_policy),
        //             std::bind(&ImageProjection::CustomHandler, this, std::placeholders::_1));

        // pubExtractedCloud = create_publisher<sensor_msgs::msg::PointCloud2>( "sc_liorf_localization/deskew/cloud_deskewed", QosPolicy(history_policy, reliability_policy));

        // pubLaserCloudInfo = create_publisher<sc_liorf_localization::msg::CloudInfo>("sc_liorf_localization/deskew/cloud_info", QosPolicy(history_policy, reliability_policy));
        ...
        subCustomCloud = create_subscription<livox_ros_driver2::msg::CustomMsg>(pointCustomTopic,qos_lidar,
                    std::bind(&ImageProjection::CustomHandler, this, std::placeholders::_1));

        pubExtractedCloud = create_publisher<sensor_msgs::msg::PointCloud2>( "sc_liorf_localization/deskew/cloud_deskewed", 1);

        pubLaserCloudInfo = create_publisher<sc_liorf_localization::msg::CloudInfo>("sc_liorf_localization/deskew/cloud_info", qos);
    ...
    void allocateMemory()
    {
        ...
        customCloudIn.reset(new pcl::PointCloud<LiovxPointCustomMsg>());
        ...
        if(sensor == SensorType::LIVOX)
        {
            ...
            tmpLivoxCMCloudIn.reset(new pcl::PointCloud<LiovxPointCustomMsg>());
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imuMsg)
    {
        if(sensor == SensorType::LIVOX)
        {
            sensor_msgs::msg::Imu thisImu = imuMid360Converter(*imuMsg);
    ...
    void CustomHandler(const livox_ros_driver2::msg::CustomMsg::SharedPtr laserCloudMsg)
    {
        if (!cacheCustomCloud(laserCloudMsg))
            return;
    ...
    void moveFromCustomMsg(livox_ros_driver2::msg::CustomMsg &Msg, pcl::PointCloud<LiovxPointCustomMsg> &cloud)
    {
        cloud.clear(); 
    ...
    bool cacheCustomCloud(const livox_ros_driver2::msg::CustomMsg::SharedPtr& laserCloudMsg)
    {
        // cache point cloud
        CustomQueue.push_back(*laserCloudMsg);
        if (CustomQueue.size() <= 2)
            return false;

        // convert cloud
        currentCustomMsg = std::move(CustomQueue.front());
        CustomQueue.pop_front();
        if (sensor == SensorType::LIVOX){
            moveFromCustomMsg(currentCustomMsg, *customCloudIn);
        }
        else
        {
            RCLCPP_ERROR_STREAM(get_logger(), "Unknown sensor type: " << int(sensor));
        ...
        // get timestamp
        cloudHeader = currentCustomMsg.header;
        timeScanCur = ROS_TIME(cloudHeader.stamp);
        timeScanEnd = timeScanCur + customCloudIn->points.back().time;
...
void imuDeskewInfo()
{
    ...
    for (int i = 0; i < (int)imuQueue.size(); ++i)
    {
        ...
        if (imuType || sensor == SensorType::LIVOX) {
    ...
    void projectCustomCloud()
    {
        int cloudSize = customCloudIn->points.size();
        // range image projection
        for (int i = 0; i < cloudSize; ++i)
        {
            PointType thisPoint;
            thisPoint.x = customCloudIn->points[i].x;
            thisPoint.y = customCloudIn->points[i].y;
            thisPoint.z = customCloudIn->points[i].z;
            thisPoint.intensity = customCloudIn->points[i].intensity;

            float range = pointDistance(thisPoint);
            if (range < lidarMinRange || range > lidarMaxRange)
                continue;

            int rowIdn = customCloudIn->points[i].ring;
            if (rowIdn < 0 || rowIdn >= N_SCAN)
                continue;
            ...
            thisPoint = deskewPoint(&thisPoint, customCloudIn->points[i].time);
...
int main(int argc, char** argv)
{
    ...
    std::string node_name = "slamMode_node";
    ParamServer paramServer(node_name, options);
    common_lib_ = std::make_shared<CommonLib::common_lib>(paramServer.slamMode);
