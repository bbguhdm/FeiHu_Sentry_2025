...
public:
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("sc_liorf_localization_mapOptimization", options)
    {
        ...
        // pubKeyPoses = create_publisher<sensor_msgs::msg::PointCloud2>("sc_liorf_localization/mapping/trajectory", QosPolicy(history_policy, reliability_policy));
        // pubLaserCloudSurround = create_publisher<sensor_msgs::msg::PointCloud2>("sc_liorf_localization/mapping/map_global", QosPolicy(history_policy, reliability_policy));
        // pubLaserOdometryGlobal = create_publisher<nav_msgs::msg::Odometry>("sc_liorf_localization/mapping/odometry", QosPolicy(history_policy, reliability_policy));
        // pubLaserOdometryIncremental = create_publisher<nav_msgs::msg::Odometry>("sc_liorf_localization/mapping/odometry_incremental", QosPolicy(history_policy, reliability_policy));
        // pubPath = create_publisher<nav_msgs::msg::Path>("sc_liorf_localization/mapping/path", QosPolicy(history_policy, reliability_policy));
        // pubHistoryKeyFrames = create_publisher<sensor_msgs::msg::PointCloud2>("sc_liorf_localization/mapping/icp_loop_closure_history_cloud", QosPolicy(history_policy, reliability_policy));
        // pubIcpKeyFrames = create_publisher<sensor_msgs::msg::PointCloud2>("sc_liorf_localization/mapping/icp_loop_closure_corrected_cloud", QosPolicy(history_policy, reliability_policy));
        // pubLoopConstraintEdge = create_publisher<visualization_msgs::msg::MarkerArray>("/sc_liorf_localization/mapping/loop_closure_constraints", QosPolicy(history_policy, reliability_policy));
        // pubRecentKeyFrames = create_publisher<sensor_msgs::msg::PointCloud2>("sc_liorf_localization/mapping/map_local", QosPolicy(history_policy, reliability_policy));
        // pubRecentKeyFrame = create_publisher<sensor_msgs::msg::PointCloud2>("sc_liorf_localization/mapping/cloud_registered", QosPolicy(history_policy, reliability_policy));
        // pubCloudRegisteredRaw = create_publisher<sensor_msgs::msg::PointCloud2>("sc_liorf_localization/mapping/cloud_registered_raw", QosPolicy(history_policy, reliability_policy));
        // pubSLAMInfo = create_publisher<sc_liorf_localization::msg::CloudInfo>("sc_liorf_localization/mapping/slam_info", QosPolicy(history_policy, reliability_policy));
        // pubGpsOdom = create_publisher<nav_msgs::msg::Odometry>("sc_liorf_localization/mapping/gps_odom", QosPolicy(history_policy, reliability_policy));
        ...
        // string saveScDirectory;
        // // saveScDirectory = std::getenv("HOME") + savePCDDirectory; // rather use global path 
    ...
    void laserCloudInfoHandler(const sc_liorf_localization::msg::CloudInfo::SharedPtr msgIn)
    {
        ...
        rclcpp::Time stamp(cloudInfo.header.stamp);
        laserCloudRawTime = stamp.seconds();
    ...
    void visualizeGlobalMapThread()
    {
        ...
        // string saveScDirectory_;
        // saveScDirectory_ = std::getenv("HOME") + savePCDDirectory; 
    ...
    void performRSLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        ndt_omp.setResolution(ndtResolution);
        ndt_omp.setTransformationEpsilon(ndtTransformationEpsilon);
        ndt_omp.setStepSize(ndtStepSize); 
        ndt_omp.setInputSource(cureKeyframeCloud);
        ndt_omp.setInputTarget(prevKeyframeCloud);

        pcl::PointCloud<PointType>::Ptr unused_result_0(new pcl::PointCloud<PointType>());
        ndt_omp.align(*unused_result_0); 
        ...
        eigen_isometry.matrix() = ndt_omp.getFinalTransformation().cast<double>();
        ...
        if (pubIcpKeyFrames->get_subscription_count() != 0)
        {
            ...
            pcl::transformPointCloud(*cureKeyframeCloud, *closed_cloud, small_gicp_result.T_target_source.cast<float>());
        ...
        float noiseScore = small_gicp_result.error;
    ...
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        std::vector<int> timeDiffCandidates; 
        ...
        for (int i = 0; i < (int)copy_cloudKeyPoses2D->size(); i++) 
            copy_cloudKeyPoses2D->points[i].z = 1.1; 

        kdtreeHistoryKeyPoses->setInputCloud(copy_cloudKeyPoses2D); 
        kdtreeHistoryKeyPoses->radiusSearch(copy_cloudKeyPoses2D->back(), historyKeyframeSearchRadius, pointSearchIndLoop, pointSearchSqDisLoop, 0); 
        ...
        for (int i = 0; i < (int)pointSearchIndLoop.size(); ++i)
        {
            ...
            {
                timeDiffCandidates.push_back(id); 
                candidateDistances.push_back(pointSearchSqDisLoop[i]); 
                if (timeDiffCandidates.size() >= 3) 
                    {
                        break; 
                    }
            }
        ...
        for (int i = 0; i < (int)timeDiffCandidates.size(); ++i) {
            if (candidateDistances[i] < minDistance) {
                minDistance = candidateDistances[i];
                loopKeyPre = timeDiffCandidates[i];
            }
        }
        ...
        //     if (abs(copy_cloudKeyPoses6D->points[id].time - timeLaserInfoCur) > historyKeyframeSearchTimeDiff)
        //     {
        //         minDistance = pointSearchSqDisLoop[i];
        //         loopKeyPre = id;
        //         break;
        //     }
        // }
                                        ...
                                        const int _wrt_key)
    {
        ...
        for (int i = -searchNum; i <= searchNum; ++i)
        {
            ...
            *nearKeyframes += *transformPointCloud(cornerCloudKeyFrames[keyNear], 
                                                &copy_cloudKeyPoses6D->points[_wrt_key]);
    ...
    void updateInitialGuess()
    {
        ...
        if (cloudKeyPoses3D->points.empty()) //首次运行
        {
