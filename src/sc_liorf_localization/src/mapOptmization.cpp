...
public:
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("sc_liorf_localization_mapOptimization", options)
    {
        ...
        subCloud = create_subscription<sc_liorf_localization::msg::CloudInfo>("sc_liorf_localization/deskew/cloud_info", qos_lidar,
    ...
    void visualizeGlobalMapThread()
    {
        ...
        std::shared_ptr<sc_liorf_localization::srv::SaveMap::Request> req = std::make_unique<sc_liorf_localization::srv::SaveMap::Request>();
        std::shared_ptr<sc_liorf_localization::srv::SaveMap::Response> res = std::make_unique<sc_liorf_localization::srv::SaveMap::Response>();
    ...
    // 更新初始位姿估计，融合IMU和里程计数据提升位姿估计精度
    void updateInitialGuess()
    {
        // 在处理前保存当前变换矩阵，用于后续增量计算
        incrementalOdometryAffineFront = trans2Affine3f(transformTobeMapped);

        static Eigen::Affine3f lastImuTransformation; // 保存上一帧的IMU变换矩阵

        /* 系统初始化场景处理 */
        if (cloudKeyPoses3D->points.empty()) // 关键帧位姿队列为空时（首次运行）
        {
            // 使用IMU初始数据初始化姿态角（roll, pitch, yaw）
            transformTobeMapped[0] = cloudInfo.imurollinit;
            transformTobeMapped[1] = cloudInfo.imupitchinit;
            transformTobeMapped[2] = cloudInfo.imuyawinit;

            // 如果不使用IMU航向初始化，将yaw重置为0
        ...
        /* 使用IMU预积分数据进行位姿估计 */
        static bool lastImuPreTransAvailable = false;      // 标记是否存在上一帧预积分变换
        static Eigen::Affine3f lastImuPreTransformation;   // 保存上一帧预积分变换
        if (cloudInfo.odomavailable == true) // 存在可用里程计数据时
        {
            // 根据当前里程计数据创建变换矩阵
            ...
            if (!lastImuPreTransAvailable) // 首次处理预积分数据
            {
                lastImuPreTransformation = transBack;  // 初始化上一帧变换
                lastImuPreTransAvailable = true;       // 标记预积分数据可用
            }
            else
            {
                // 计算两帧之间的增量变换：上一帧的逆变换 * 当前变换
                ...
                // 分解变换矩阵为欧拉角和平移量，更新位姿估计
                pcl::getTranslationAndEulerAngles(
                    transFinal,
                    transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5], // 平移分量(x,y,z)
                    transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]  // 旋转分量(roll,pitch,yaw)
                );

                lastImuPreTransformation = transBack; // 更新上一帧预积分变换
                lastImuTransformation = pcl::getTransformation(0, 0, 0, cloudInfo.imurollinit, cloudInfo.imupitchinit, cloudInfo.imuyawinit); // 更新IMU变换
                return; // 处理完成后返回
            }
        }

        /* 使用IMU增量旋转估计（仅旋转部分）*/
        if (cloudInfo.imuavailable == true && imuType) // IMU数据可用且类型匹配时
        {
            // 创建当前IMU旋转矩阵（平移设为0）
            Eigen::Affine3f transBack = pcl::getTransformation(0, 0, 0, cloudInfo.imurollinit, cloudInfo.imupitchinit, cloudInfo.imuyawinit);
            
            // 计算旋转增量：上一帧IMU旋转的逆 * 当前IMU旋转
            ...
            pcl::getTranslationAndEulerAngles(
                transFinal,
                transformTobeMapped[3], transformTobeMapped[4], transformTobeMapped[5], // 平移分量保持不变
                transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]  // 更新旋转分量
            );
