...
public:
    ...
    TransformFusion(const rclcpp::NodeOptions & options) : ParamServer("sc_liorf_localization_transformFusion", options)
    {
        ...
        // subLaserOdometry = create_subscription<nav_msgs::msg::Odometry>("sc_liorf_localization/mapping/odometry", QosPolicy(history_policy, reliability_policy), 
        ...
        // pubImuPath = create_publisher<nav_msgs::msg::Path>("sc_liorf_localization/imu/path", QosPolicy(history_policy, reliability_policy));

        subLaserOdometry = create_subscription<nav_msgs::msg::Odometry>("sc_liorf_localization/mapping/odometry", 1, 
                    std::bind(&TransformFusion::lidarOdometryHandler, this, std::placeholders::_1));
        ...
        pubImuPath = create_publisher<nav_msgs::msg::Path>("sc_liorf_localization/imu/path", qos);
    ...
    // 函数：odom2affine
    // 功能：将ROS 2中的Odometry消息转换为Eigen库中的Isometry3d变换矩阵
    // 参数：
    //   - odom: nav_msgs::msg::Odometry类型的消息，包含位姿信息
    // 返回值：
    //   - Eigen::Isometry3d: 表示3D空间中的刚体变换矩阵

    Eigen::Isometry3d odom2affine(nav_msgs::msg::Odometry odom) {
        // 创建一个tf2::Transform对象，用于存储从Odometry消息中提取的位姿信息
        tf2::Transform t;

        // 将Odometry消息中的pose.pose（geometry_msgs::msg::Pose类型）转换为tf2::Transform类型
        // fromMsg函数将ROS消息类型转换为tf2库中的类型
        tf2::fromMsg(odom.pose.pose, t);

        // 将tf2::Transform对象转换回ROS消息类型（geometry_msgs::msg::Transform），
        // 然后使用tf2::transformToEigen将其转换为Eigen::Isometry3d类型
        // Isometry3d是Eigen库中表示3D刚体变换的矩阵类型
...
public:
    ...
    //added *****************************  by gc
    rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr subPoseOdomToMap;
    ...
    IMUPreintegration(const rclcpp::NodeOptions & options) :
            ParamServer("sc_liorf_localization_imu_preintegration", options)
    {
        ...
        // subPoseOdomToMap = create_subscription<geometry_msgs::msg::PoseStamped>("sc_liorf_localization/mapping/pose_odomTo_map", QosPolicy(history_policy, reliability_policy), 
        //             std::bind(&IMUPreintegration::odomToMapPoseHandler, this, std::placeholders::_1));
        // subOdometry = create_subscription<nav_msgs::msg::Odometry>("sc_liorf_localization/mapping/odometry_incremental", QosPolicy(history_policy, reliability_policy),
        ...
        subImu = create_subscription<sensor_msgs::msg::Imu>(imuTopic, qos_imu, 
                    std::bind(&IMUPreintegration::imuHandler, this, std::placeholders::_1));
        subPoseOdomToMap = create_subscription<geometry_msgs::msg::PoseStamped>("sc_liorf_localization/mapping/pose_odomTo_map", qos_imu, 
                    std::bind(&IMUPreintegration::odomToMapPoseHandler, this, std::placeholders::_1));
        subOdometry = create_subscription<nav_msgs::msg::Odometry>("sc_liorf_localization/mapping/odometry_incremental", qos,
                    std::bind(&IMUPreintegration::odometryHandler, this, std::placeholders::_1));
        ...
        rotation.setRPY(0, 0, 0);  // set roll, pitch, yaw (0, 0, 0)
        tf2::Vector3 translation(0, 0, 0);  // translation (x, y, z)

        map_to_odom.setRotation(rotation);
    ...
    void odomToMapPoseHandler(const geometry_msgs::msg::PoseStamped::ConstSharedPtr poseOdomToMapmsg)

    {
        ...
        map_to_odom    = tf2::Transform(q_tem, p_tem);
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
    {
        ...
        {
            sensor_msgs::msg::Imu thisImu = imuMid360Converter(*imu_raw);

            geometry_msgs::msg::TransformStamped transformStamped;
            transformStamped.header.stamp = thisImu.header.stamp;
            transformStamped.header.frame_id = "map";
            transformStamped.child_frame_id = "odom";
            transformStamped.transform.translation.x = map_to_odom.getOrigin().x();
            transformStamped.transform.translation.y = map_to_odom.getOrigin().y();
            transformStamped.transform.translation.z = map_to_odom.getOrigin().z();
            transformStamped.transform.rotation.x = map_to_odom.getRotation().x();
            transformStamped.transform.rotation.y = map_to_odom.getRotation().y();
            transformStamped.transform.rotation.z = map_to_odom.getRotation().z();
            transformStamped.transform.rotation.w = map_to_odom.getRotation().w();
