...
// 重载
std::pair<int, float> SCManager::detectLoopClosureID (const int& subsequent_invkeys_mat)
{
    int subsequent_invkeys_mat_ = subsequent_invkeys_mat;
    ...
    if( tree_making_period_conter % TREE_MAKING_PERIOD_ == 0) 
    {
        ...
        polarcontext_invkeys_to_search_.clear();
        polarcontext_invkeys_to_search_.assign( polarcontext_invkeys_mat_.begin(), polarcontext_invkeys_mat_.end() - subsequent_invkeys_mat_ ) ;
